  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        console.log(`Fetching product with ID: ${id}`);
        // Fetch product details by ID
        const response = await axios.get(`https://cakes-backend-q803.onrender.com/api/dessert/${id}`);
        console.log("Response data:", response.data);
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 mt-10">
          <motion.img
            ...
            animate={{ x: 0, opacity: 1 }}
            transition={{ duration: 1 }}
            src={`https://cakes-backend-q803.onrender.com/upload/${mainImage}`}
            alt={product.name || "Product"}
            className="w-full h-full lg:w-8/12 lg:h-8/12 mx-auto object-cover rounded-xl shadow-md"
        ...
        <div className="flex-1 lg:mt-20">
          ...
          <div className="flex gap-4 mt-4">
            {product.addImage1 && (
              <img
                src={`https://cakes-backend-q803.onrender.com/upload/${product.addImage1}`}
                alt="Additional view 1"
                className="w-20 h-20 object-cover rounded shadow-md cursor-pointer"
                onClick={() => setMainImage(product.addImage1)}
            ...
            {product.addImage2 && (
              <img
                src={`https://cakes-backend-q803.onrender.com/upload/${product.addImage2}`}
                alt="Additional view 2"
                className="w-20 h-20 object-cover rounded shadow-md cursor-pointer"
                onClick={() => setMainImage(product.addImage2)}
              // Fallback for missing images
